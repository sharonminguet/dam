...
def procesar():
    ...
    if not entrada or not salida:
        print("Por favor, selecciona archivos de entrada y salida.")
        return
    
    print("Vamos a por ello")
    command = f'ffmpeg -i "{entrada}" -vf scale={anchura.get()}:{altura.get()} "{salida}"'
    print(command)
    ...
    try:
        ...
        print("Error al ejecutar FFmpeg:", e.stderr)
...
def seleccionaSalida():
    global salida
    salida = filedialog.asksaveasfilename(defaultextension=".mp4", filetypes=[("MP4 files", "*.mp4")])
